...
def lang_convert(language_reference, lang):
    """
    This function takes the dict variable language_reference
    which has the two digit language field and the corresponding
    3 digit language field. and uses that to convert the input
    lang variable which is a 2 digit language reference from the
    Gracenote ONTV format to convert it to the 3 digit field
    required
    """
    try:
        ulang = language_reference[lang]
        return ulang
    except:
        return ''
...
def get_service_info(channel, source_filename, language_reference):
    ...
    service_info["lang"] = lang_convert(language_reference, element['bcastLangs']['bcastLang'])
    return service_info
...
def insert_service_info_table(service_information_table, service_info):
    ...
    service_information = objectify.SubElement(service_information_table, "ServiceInformaStion")
    service_information.set("serviceId", service_info["serviceId"])
    service_information.Name = ""
    service_information.Name._setText(etree.CDATA(service_info["callSign"]))
    try:
...
def create_schedule(event_data, b_d_unique):
    ...
    schedule = dict(
        ...
        ScheduleScheduleEventProgram_att_crid=b_d_unique['program_id'],
...
def fill_program_location_table(program_location_table, event_data, b_d_unique):
    ...
    """
    schedule = create_schedule(event_data, b_d_unique)
...
def handle_event(main_tables, event_data, program_ids_already_used, group_ids_already_used,
    ...
    if get_gracenote_season_id(event_data['program_id'], 'seriesId') != '':
        series_id = 'crid://'+ service_info['callSign'] + '/' +\
            get_gracenote_season_id(event_data['program_id'], 'seriesId')
    else:
        series_id = ''
    if get_gracenote_season_id(event_data['program_id'], 'seasonId') != '':
        season_id = 'crid://'+ service_info['callSign'] + '/' +\
            get_gracenote_season_id(event_data['program_id'], 'seasonId')
    else:
        season_id = ''
    b_d_unique = []
    b_d_unique = create_b_d_unique(grace_note_dict, event_data, season_id,
                                   service_info, language_reference)
    b_d_credits = []
#    credits_list_item = []
    b_d_credits = create_b_d_credits(grace_note_dict)
#    b_d_synopsis = []
    b_d_synopsis = create_b_d_synopsis(grace_note_dict, language_reference)
...
#
    ...
    program_information_element =\
        build_program_information(main_tables["program_information_table"],
                                  b_d_unique['program_id'], b_d_unique['guid'])
    build_basic_description_unique(grace_note_dict, program_information_element,
                                   season_id, b_d_unique, b_d_credits, b_d_synopsis,
                                   b_d_genres, graphics_assets, language_reference)

    build_avattributes(program_information_element)
    ...
    if match != 1:
        ...
        fill_group_table(main_tables["group_information_table"],
                         ...
                         b_d_unique, graphics_assets, language_reference)
...
def finally_write_xml_file(root_of_objectivy_object, filename, BASEDIRECTORY, cleanup_namespaces,
    ...
    objectify.deannotate(root_of_objectivy_object,
                         cleanup_namespaces=cleanup_namespaces, xsi_nil=True)
...
def create_b_d_unique(grace_note_dict, event_data, season_id, service_info, language_reference):
    ...
    """
    b_d_unique = dict()
    b_d_unique['program_id'] = "crid://"+service_info['callSign']+"/"+event_data['program_id']+\
        "." + event_data['gmt_date_time_start_zw2'][0:14]+\
        '.'+service_info['serviceId']
    b_d_unique['guid'] = event_data['program_id']+\
        "." + event_data['gmt_date_time_start_zw2'][0:14]+\
        '.'+service_info['serviceId']
    try:
        ...
        if isinstance(grace_note_dict['xml']['ratings']['rating'], list):
            ...
            for element in grace_note_dict['xml']['ratings']['rating']:
                if element['@area'] == 'USA Parental Rating':
                    ...
                    check = 1
            if check == 0:
                b_d_unique['BDParentalGuidanceMinimumAge'] = '0'
...
#call it each time
    ...
    try:
        if   isinstance(grace_note_dict['xml']['titles']['title'], list):
            ...
            b_d_unique['BDTitle_att_lang'] = lang_convert(language_reference,\
                          grace_note_dict['xml']['titles']['title'][0]['@lang'])
            b_d_unique['BDTitle_att_lang_zw25'] = grace_note_dict\
                ['xml']['titles']['title'][0]['@lang']
            #b_d_unique['BDTitle_att_type'] = grace_note_dict['xml']['titles']['title'][0]['@type']
            b_d_unique['BDTitle_att_type'] = "original"
        else:
            b_d_unique['BDTitle'] = grace_note_dict['xml']['titles']['title']['#text']
            b_d_unique['BDTitle_att_lang'] = lang_convert\
                (language_reference, grace_note_dict['xml']['titles']['title']['@lang'])
            b_d_unique['BDTitle_att_lang_zw25'] = grace_note_dict\
                ['xml']['titles']['title'][0]['@lang']
        ...
        b_d_unique['BDTitle_att_lang_zw25'] = ""
        b_d_unique['BDTitle_att_type'] = ""
    try:
        b_d_unique["programEpisodeTitle"] = grace_note_dict\
            ['xml']['episodeInfo']['title']['#text']
        b_d_unique["programEpisodeTitle_att_lang"] =\
                lang_convert(language_reference,
...
def create_b_d_synopsis(grace_note_dict, language_reference):
    """This function takes the xmltodict list variable which was created from the
        ...
        if   isinstance(grace_note_dict['xml']['descriptions']['desc'], list):
            for index, element in enumerate(grace_note_dict['xml']['descriptions']['desc']):
                ...
                b_d_synopsis.append({
                    ...
                    "b_d_synopsis_att_lang" : lang_convert(language_reference, element['@lang']),
                    "b_d_synopsis_att_lang_zw25" : element['@lang'],
            ...
            for element in grace_note_dict['xml']['assets']['asset']:
                ...
                and (element['@tier'] != 'Season' or  element['@tier'] != 'Series'):
                    ...
                    b_d_related.append({
                        ...
                        "BDMediaLocatorMediaUri" : remove_leading_directory(element['URI'])
                        })
...
def create_group_info_series_title(grace_note_dict, language_reference):
    """This function takes the xmltodict list variable which was created from the
    xml data for the specific programID, along with seriedID and the season_id and
    generates a specific dict variable which has the information required for the
        ...
        if isinstance(grace_note_dict['xml']['episodeInfo']['title'], list):
            for element in grace_note_dict['xml']['episodeInfo']['title']:
                group_info_series_title.append({
                    ...
                    "GinfoBasicDescriptionTitle_att_lang" : lang_convert\
                        (language_reference, element['@lang']),
...
def build_basic_description_unique(grace_note_dict, program_information_element,
    ...
    if b_d_unique['programEpisodeTitle'] == '':
        bd_title1 = objectify.SubElement(basic_description, "Title")
        bd_title1._setText(etree.CDATA(b_d_unique["BDTitle"]))
        bd_title1.set("{http://www.w3.org/XML/1998/namespace}lang",
                      b_d_unique["BDTitle_att_lang"])
        bd_title2 = objectify.SubElement(basic_description, "Title")
        bd_title2._setText(etree.CDATA(b_d_unique["BDTitle"]))
        bd_title2.set("type", b_d_unique["BDTitle_att_type"])
...
#        basic_description.Title.set("type", b_d_unique["BDTitle_att_type"])
        ...
        bd_title1 = objectify.SubElement(basic_description, "Title")
        bd_title1._setText(etree.CDATA(b_d_unique["programEpisodeTitle"]))
        bd_title1.set("{http://www.w3.org/XML/1998/namespace}lang",
                      b_d_unique["programEpisodeTitle_att_lang"])
        bd_title2 = objectify.SubElement(basic_description, "Title")
        bd_title2._setText(etree.CDATA(b_d_unique["programEpisodeTitle"]))
        bd_title2.set("type", b_d_unique["BDTitle_att_type"])
...
#                                b_d_unique["BDTitle_att_lang"])
#    basic_description.Title.set("type", b_d_unique["BDTitle_att_type"])
    basic_description.Keyword = ''
    basic_description.Keyword._setText(etree.CDATA(b_d_unique["BDKeyword"]))
...
#    b_d_synopsis = create_b_d_synopsis(grace_note_dict)
    for index, b_d_synopsis_element in enumerate(b_d_synopsis):
        ...
        synopsis[index]._setText(etree.CDATA(b_d_synopsis_element["b_d_synopsis"]))
    ...
    for b_d_credits_element in b_d_credits:
        if b_d_credits_element["BDPersonNameRole"].upper() != "PRODUCER":
            credits_list_item_temp = objectify.SubElement(basic_description.CreditsList,
                                                          "CreditsItem")
            credits_item.append(credits_list_item_temp)
            #credits_list_item[index].CreditsItem = ""
            credits_item[index].set("role", b_d_credits_element["b_d_creditsListCreditsItem_att_role"])
            credits_item[index].PersonName = ''
            objectify.SubElement(credits_item[index].PersonName,
                                 "{urn:tva:mpeg7:2008}GivenName").\
                                 _setText(etree.CDATA(b_d_credits_element["BDPersonNameGivenName"]))
            objectify.SubElement(credits_item[index].PersonName,
                                 "{urn:tva:mpeg7:2008}FamilyName").\
                                 _setText(etree.CDATA(b_d_credits_element\
                                                      ["BDPersonNameFamilyName"]))
            index = index + 1
#    credits_item = []
#    for b_d_credits_element in b_d_credits:
#        if b_d_credits_element["BDPersonNameRole"].upper() != "PRODUCER":
#            basic_description.CreditsList.CreditsItem[index] = ''
#            basic_description.CreditsList.CreditsItem.set("role", b_d_credits_element["b_d_creditsListCreditsItem_att_role"])
##            credits_item.append(credits_item_temp)
##            credits_item[index].\
##                    set("role", b_d_credits_element["b_d_creditsListCreditsItem_att_role"])
#            basic_description.CreditsList.CreditsItem.PersonName = ''
#            objectify.SubElement(basic_description.CreditsList.CreditsItem.PersonName,
#                                 "{urn:tva:mpeg7:2008}GivenName").\
#                                 _setText(etree.CDATA(b_d_credits_element["BDPersonNameGivenName"]))
#            objectify.SubElement(basic_description.CreditsList.CreditsItem.PersonName,
#                                 "{urn:tva:mpeg7:2008}FamilyName").\
...
def build_group_info_series_unique(group_information_table, grace_note_dict,
                                   series_id, season_id, b_d_unique, language_reference):
    ...
    group_info_series_unique = create_group_info_series_unique(grace_note_dict, series_id,
                                                               season_id)
    group_info_series_title = create_group_info_series_title(grace_note_dict, language_reference)
    ...
    try:
        for group_title_element in group_info_series_title:
            group_information.BasicDescription.Title._setText\
                (etree.CDATA(group_title_element["GinfoBasicDescriptionTitleValue"]))
            group_information.BasicDescription.Title.\
...
def build_program_information(program_information_table, program_id, guid):
    ...
    program_information_element.set("{urn:zw:sds-evo:2016}guid", guid)
    return program_information_element


def fill_group_table(group_information_table, grace_note_dict, series_id, season_id,
                     group_ids_already_used, b_d_unique, graphics_assets, language_reference):
    """This function takes the lxml objectify object for the Program Information
    ...
    build_group_info_series_unique(group_information_table, grace_note_dict, series_id,
                                   season_id, b_d_unique, language_reference)
    if season_id != '':
        match = 0
        for element in group_ids_already_used:
...
def build_avattributes(program_information_element):
    """This function adds in fixed Audio Video attributes
    as these values are not available in the GraceNote
    OnTV format
    """
    avattrib = objectify.SubElement(program_information_element, "AVAttributes")
    avattrib.VideoAttributes = ""
    avattrib.VideoAttributes.AspectRatio = ''
    avattrib.VideoAttributes.AspectRatio._setText('16:9')
    avattrib.VideoAttributes.PictureFormat = ''
    avattrib.VideoAttributes.PictureFormat.set\
        ('href', 'urn:tva:metadata:cs:PictureFormatCS:2015:1.2')
...
def add_cdata(input_string):
    """no longer needed as it is possible to add CDATA wrapper
    to lxml objecte using etree.CDATA function
    """
    if input_string != '':
        return '<![CDATA[ '+ input_string+' ]]>'
    else:
        return input_string

def remove_leading_directory(input_string):
    """note changing from assets/filename.jpg to ./filename.jpg
    """
    if input_string != '':
